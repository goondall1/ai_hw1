...
def plot_distance_and_expanded_wrt_weight_figure(
    ...
    p1, = ax1.plot(weights, total_cost, label='Solution Cost', color='b', linestyle='solid')
...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    res_g_costs, res_expands, res_weights = [], [], []
    for w in np.linspace(low_heuristic_weight, high_heuristic_weight, n):
        Astar_w_weights = AStar(heuristic_type, w, max_nr_states_to_expand)
        res = Astar_w_weights.solve_problem(problem)
        if res.is_solution_found:
            res_g_costs.append(res.solution_g_cost)
            res_expands.append(res.nr_expanded_states)
            res_weights.append(w)

    plot_distance_and_expanded_wrt_weight_figure(problem.name, res_weights, res_g_costs, res_expands)
...
def toy_map_problem_experiments():
    ...
    weighted_astar = AStar(NullHeuristic)
    res = weighted_astar.solve_problem(toy_map_problem)
    print(res)

    # Ex.11
    # TODO: create an instance of `AStar` with the `AirDistHeuristic`,
    #       solve the same `toy_map_problem` with it and print the results (as before).
    weighted_astar = AStar(AirDistHeuristic)
    res = weighted_astar.solve_problem(toy_map_problem)
    print(res)
...
def mda_problem_with_astar_experiments():
    ...
    weighted_astar = AStar(MDAMaxAirDistHeuristic)
    res = weighted_astar.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)

    # Ex.20
    # TODO: create an instance of `AStar` with the `MDASumAirDistHeuristic`,
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    weighted_astar = AStar(MDASumAirDistHeuristic)
    res = weighted_astar.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
    ...
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    weighted_astar = AStar(MDAMSTAirDistHeuristic)
    res = weighted_astar.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
...
def multiple_objectives_mda_problem_experiments():
    ...
    weighted_astar = AStar(MDATestsTravelDistToNearestLabHeuristic)
    res = weighted_astar.solve_problem(moderate_mda_problem_with_tests_travel_dist_cost)
    print(res)
    ...
    #       Solve the `moderate_mda_problem_with_tests_travel_dist_cost` with it and print the results.
    eps = 0.6
    astar_dist_air_huristic = AStar(MDAMSTAirDistHeuristic)
    res_dist = astar_dist_air_huristic.solve_problem(moderate_mda_problem_with_distance_cost)
    optimal_distance_cost = res_dist.solution_cost.get_g_cost()
    max_distance_cost = optimal_distance_cost * (1 + eps)
    astar_test_travel_huristic = AStar(MDATestsTravelDistToNearestLabHeuristic,
                                       open_criterion=lambda node: node.operator_cost.get_g_cost() < max_distance_cost)
    res = astar_test_travel_huristic.solve_problem(moderate_mda_problem_with_tests_travel_dist_cost)
    print(res)
...
def mda_problem_with_astar_epsilon_experiments():
    ...
    aster_eps = AStarEpsilon(MDAMSTAirDistHeuristic, within_focal_priority_function=within_focal_h_sum_priority_function,
                             focal_epsilon=0.03, max_focal_size=40)
    res = aster_eps.solve_problem(small_mda_problem_with_distance_cost)
    print(res)
...
def mda_problem_anytime_astar_experiments():
    ...
    anytimeAstar = AnytimeAStar(MDAMSTAirDistHeuristic,150)
