...
class MDAMaxAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        all_certain_junctions_in_remaining_ambulance_path.append(state.current_location)
        return max(junction1.calc_air_distance_from(junction2) for junction1 in
                   all_certain_junctions_in_remaining_ambulance_path
                   for junction2 in all_certain_junctions_in_remaining_ambulance_path if junction1 != junction2)
...
class MDASumAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        if curr in all_certain_junctions_in_remaining_ambulance_path:
        ...
        while len(all_certain_junctions_in_remaining_ambulance_path):
            for junction in all_certain_junctions_in_remaining_ambulance_path:
                dist = self.cached_air_distance_calculator.get_air_distance_between_junctions(curr,
                                                                                              junction) if junction != curr else dist
                if dist < min_dist:
...
class MDAMSTAirDistHeuristic(HeuristicFunction):
    ...
    def _calculate_junctions_mst_weight_using_air_distance(self, junctions: List[Junction]) -> float:
       ...
       """
        ...
        G.add_weighted_edges_from((junction1, junction2,
                                   self.cached_air_distance_calculator.get_air_distance_between_junctions(junction1,
                                                                                                          junction2))
                                  for junction1 in junctions for junction2 in junctions if junction1 != junction2)
...
class MDATestsTravelDistToNearestLabHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        return air_dist_to_closest_lab(
            state.current_location) * state.get_total_nr_tests_taken_and_stored_on_ambulance() + \
               sum(air_dist_to_closest_lab(apt.location) * apt.nr_roommates \
