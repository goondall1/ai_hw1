...
class MDAState(GraphProblemState):
    ...
    This state includes:
        `current_site`:
            The current site where the ambulance is at.
...
class MDAProblem(GraphProblem):
    ...
    def expand_state_with_costs(self, state_to_expand: GraphProblemState) -> Iterator[OperatorResult]:
        ...
        possible_labs = set(self.problem_input.laboratories)
        if state_to_expand.get_total_nr_tests_taken_and_stored_on_ambulance() == 0:
            possible_labs -= state_to_expand.visited_labs
        lab: Laboratory
        for lab in possible_labs:
            operator_name = "go to lab " + lab.name
            visited = state_to_expand.visited_labs.__contains__(lab)
            new_matoshim = state_to_expand.nr_matoshim_on_ambulance
            if not visited:
                new_matoshim += lab.max_nr_matoshim
            successor_state = MDAState(lab, frozenset(),
                                       (state_to_expand.tests_on_ambulance | state_to_expand.tests_transferred_to_lab),
                                       new_matoshim, (frozenset([lab]) | state_to_expand.visited_labs))
            yield OperatorResult(successor_state, self.get_operator_cost(state_to_expand, successor_state),
                                 operator_name)

        possible_apartments = (frozenset(self.problem_input.reported_apartments) - (
                    state_to_expand.tests_transferred_to_lab | state_to_expand.tests_on_ambulance))
        for apartment in possible_apartments:
            if apartment.nr_roommates <= state_to_expand.nr_matoshim_on_ambulance and apartment.nr_roommates <= self.problem_input.ambulance.taken_tests_storage_capacity - state_to_expand.get_total_nr_tests_taken_and_stored_on_ambulance():
                operator_name = "visit " + apartment.reporter_name
                new_matoshim = state_to_expand.nr_matoshim_on_ambulance - apartment.nr_roommates
                successor_state = MDAState(apartment, (state_to_expand.tests_on_ambulance | frozenset([apartment])),
                                           state_to_expand.tests_transferred_to_lab, new_matoshim,
                                           state_to_expand.visited_labs)
                yield OperatorResult(successor_state, self.get_operator_cost(state_to_expand, successor_state), operator_name)
    ...
    def get_operator_cost(self, prev_state: MDAState, succ_state: MDAState) -> MDACost:
        ...
        return MDACost(distance_cost, distance_cost * prev_state.get_total_nr_tests_taken_and_stored_on_ambulance(),
    ...
    def is_goal(self, state: GraphProblemState) -> bool:
        ...
        return state.tests_transferred_to_lab == frozenset(self.problem_input.reported_apartments) \
               ...
               and state.nr_matoshim_on_ambulance >= 0 \
    ...
    def get_reported_apartments_waiting_to_visit(self, state: MDAState) -> Set[ApartmentWithSymptomsReport]:
        ...
        """
        return set(self.problem_input.reported_apartments).difference(state.tests_on_ambulance).difference(state.tests_transferred_to_lab)
    ...
    def get_all_certain_junctions_in_remaining_ambulance_path(self, state: MDAState) -> List[Junction]:
        ...
        """
        waiting_apts = self.get_reported_apartments_waiting_to_visit(state)
        junctions = list(apartment.location for apartment in waiting_apts)
        junctions.append(state.current_location)
        return sorted(junctions, key=lambda junction: junction.index)
