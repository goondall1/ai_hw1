...
class CachedMapDistanceFinder:
    ...
    def get_map_cost_between(self, src_junction: Junction, tgt_junction: Junction) -> Optional[Cost]:
        ...
        """
        src_id, tgt_id = src_junction.index, tgt_junction.index
        if self._is_in_cache((src_id, tgt_id)):
            return self._get_from_cache((src_id, tgt_id))
        map_prob = MapProblem(self.streets_map, src_id, tgt_id)
        assert isinstance(self.map_problem_solver, GraphProblemSolver)
        res = self.map_problem_solver.solve_problem(map_prob)
        self._insert_to_cache((src_id, tgt_id), res.solution_cost)
